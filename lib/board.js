...
import { bytesToHex, hexToBytes, sha256Hex, timingSafeEqual, clientIp } from './auth.js';
...
/* 글 비밀번호 파생 횟수.
   ...
   지키는 대상이 "남의 게시글 삭제"라 이 정도로 충분하고, 화면에서도
   사이트 비밀번호를 재사용하지 말라고 안내합니다. */
...
export async function mayModify(row, body, session) {
...
export function publicPost(row, { withBody = false } = {}) {
  const out = {
    id:           row.id,
    title:        row.title,
